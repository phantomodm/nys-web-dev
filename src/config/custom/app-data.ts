const cdn_url = 'https://res.cloudinary.com/aviabird/image/upload/q_60/v1529433193/nys-web-dev/';

export const CUSTOM_APP_DATA = {
  landing_page_banner: [
    {
      image_link: `${cdn_url}banner-1.jpg`,
      link_url: '#'
    },
    {
      image_link: `${cdn_url}banner-2.jpg`,
      link_url: '#'
    },
    {
      image_link: `${cdn_url}banner-3.jpg`,
      link_url: '#'
    },
    {
      image_link: `${cdn_url}banner-4.jpg`,
      link_url: '#'
    }
  ],
  promo_banner: {
    image_link: `${cdn_url}secondary-banner-1.jpg`,
    link_url: '#'
  },
  category_banner: {
    Living: {
      image_link: `${cdn_url}banner-1.jpg`,
      link_url: '#'
    },
    Bedroom: {
      image_link: `${cdn_url}banner-2.jpg`,
      link_url: '#'
    },
    Dining: {
      image_link: `${cdn_url}banner-3.jpg`,
      link_url: '#'
    },
    Study: {
      image_link: `${cdn_url}banner-4.jpg`,
      link_url: '#'
    }
  },
  Deals: {
    type: 'Today\'s Deals'
  },

  footer_page_links: [
    {
      name: 'About Us',
      link_url: '#'
    },
    {
      name: 'Blog',
      link_url: '#'
    },
    {
      name: 'Return Policy',
      link_url: '#'
    },
    {
      name: 'FAQs',
      link_url: '#'
    },
    {
      name: 'Testimonials',
      link_url: '#'
    }
  ],
  footer_social_links: [
    {
      link_url: 'https://twitter.com/nys-web-dev',
      name: 'Twitter',
      icon: 'fa fa-twitter-square'
    },
    {
      link_url: 'https://www.instagram.com/nys-web-dev/',
      name: 'Instagram',
      icon: 'fa fa-instagram'
    },
    {
      link_url:
        'https://plus.google.com/b/110371544800340671090/110371544800340671090',
      name: 'Google +',
      icon: 'fa fa-google-plus-square'
    },
    {
      link_url: 'https://in.pinterest.com/nys-web-dev/',
      name: 'Pinterest',
      icon: 'fa fa-pinterest-square'
    },
    {
      link_url: 'https://www.facebook.com/nys-web-dev/',
      name: 'Facebook',
      icon: 'fa fa-facebook-square'
    },
    {
      link_url: 'https://www.youtube.com/channel/UCFBvY3QxKAKBAI_chAzRpjQ',
      name: 'Youtube',
      icon: 'fa fa-youtube-square'
    }
  ],
  contact_info: {
    contact_no: '888-888-xxx',
    copyright: 'Copyright © 2018 nys-web-dev, Inc.'
  }
};
